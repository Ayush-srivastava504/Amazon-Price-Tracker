...
from extract import extract_product_data, extract_live
from load_raw import load_to_raw_table
    ...
    def run(self, product_ids: Optional[list] = None):           # Main pipeline execution
        ...
        try:
            ...
            if not quality_passed:
                ...
                # In production, might want to stop here or send alert
            
            # STEP 5: LOAD TRANSFORMED (to final table)
            logger.info("=== LOAD TRANSFORMED PHASE ===")
            if self.db_conn and transformed_data:
                # This would be similar to load_to_raw_table but for final table
                # For now, just log
                logger.info(f"Would load {len(transformed_data)} transformed records to final table")
            ...
            runtime = (self.results['end_time'] - self.results['start_time']).total_seconds()
            logger.info(f"Pipeline completed in {runtime:.1f}s. Processed {len(transformed_data)} records.")
...
def main():          #Command line entry point
    ...
    # Initialize pipeline
    # Note: In real code, we'd set up DB connection here
    runner = PipelineRunner(
        db_conn=None,  # Would be real DB connection
